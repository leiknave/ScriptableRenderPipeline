...
real SampleShadow_PCSS(ShadowContext shadowContext, inout uint payloadOffset, real4 textureSize, real4 texelSizeRcp, real3 coord, real2 sampleBias, float slice, Texture2DArray tex, SamplerComparisonState samp)
{
	...
	//Naive Implementation of PCSS
	//ref: http://developer.download.nvidia.com/whitepapers/2008/PCSS_Integration.pdf
	//-------------------------------------------------------------------------------
	real2 UV 	   = coord.xy;
	real  Receiver = coord.z;

	//1) Blocker Search
	real AverageBlockerDepth = 0;
	real NumBlockers 		 = 0;

	real SearchWidth = 0.133 * (Receiver - 0.2 / Receiver);
	real BlockerSum = 0;
	
	UNITY_UNROLL
	for(int i = 0; i < 16; ++i)
	{
		real ShadowMapDepth = SAMPLE_TEXTURE2D_ARRAY_LOD(tex, sampler_point_clamp, UV, slice, 0.0).x;
...
real SampleShadow_SelectAlgorithm( ShadowContext shadowContext, ShadowData shadowData, inout uint payloadOffset, real3 posTC, real2 sampleBias, uint algorithm, Texture2DArray tex, SamplerComparisonState compSamp )
{
	...
	case GPUSHADOWALGORITHM_PCSS			: return SampleShadow_PCSS( shadowContext, payloadOffset, shadowData.textureSize, shadowData.texelSizeRcp, posTC, sampleBias, shadowData.slice, tex, compSamp );
	default: return 1.0;
	}
}
